...
/**
 * It renders a message display component, which is a component that displays a message
 * @returns A component
 */
const LeftSideMessage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoadMedia(true);
    let newArr = [];
    if (media.length > 0) newArr = await cloudinaryUpload(media);
    const msg = {
  ...
  useEffect(() => {
    ...
    getMessagesData();
  }, [id, dispatch, auth, message.data]); //Załaduj więcej
  ...
  const handleDeleteConversation = () => {
    if (window.confirm("Czy na pewno chcesz skasować?")) {
      ...
      return history.push("/komunikator");
    }
  };
  ...
  return (
    <>
      ...
      <form className="chat_input" onSubmit={handleSubmit}>
        ...
        <Emoji setContent={setText} content={text} theme={theme} />
